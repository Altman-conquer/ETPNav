...
class RLTrainer(BaseVLNCETrainer):
        ...
        for stepk in range(self.max_len):
            ...
            try:
                ...
                for i, gmap in enumerate(self.gmaps):
                    ...
                    rgb_frame = self.envs.call_at(i, "get_rgb_frame",
                                                  {"ghost_positions": new_ghost_nodes, 'flat': True})
                ...
                env_actions = []
                use_tryout = (
                        self.config.IL.tryout and not self.config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.ALLOW_SLIDING)
                for i, gmap in enumerate(self.gmaps):
                    ...
                    if (llm_result[i]['waypoint_chosen'] == 'unneeded' and current_instruction_id[i] >= len(
                    ...
                    elif (cpu_a_t[i] == 0 and llm_result[i][
                        'waypoint_chosen'] == 'unknown') or stepk == self.max_len - 1 or no_vp_left[i]:
                        # stop at node with max stop_prob
                        vp_stop_scores = [(vp, stop_score) for vp, stop_score in gmap.node_stop_scores.items()]
                        stop_scores = [s[1] for s in vp_stop_scores]
                        stop_vp = vp_stop_scores[np.argmax(stop_scores)][0]
                        ...
                        if llm_result[i]['waypoint_chosen_same_as_expert'] or llm_result[i][
                            'waypoint_chosen'] == 'unknown':
                            ghost_vp = nav_inputs['gmap_vp_ids'][i][cpu_a_t[i]]
                        ...
                        #             mylogger.error(traceback.format_exc())
                            ...
                            ghost_vp = llm_result[i]['waypoint_color_mapping'][llm_result[i]['waypoint_chosen']][
                                'vp_id']
                        ...
                        if ghost_vp is None:
                            mylogger.error(
                                f"ghost_vp is None, cpu_a_t: {cpu_a_t[i]}, ghost_vp: {ghost_vp}, waypoint_color_mapping: {llm_result[i]['waypoint_color_mapping']}")
            ...
            try:
                if use_llm:
                    ...
                    for i, gmap in enumerate(self.gmaps):
                        ...
                        try:
                            analysis = result.split("Analysis: ")[1].split("Current goal finished:")[0]
                            current_goal_finished = \
                                result.split("Current goal finished: ")[1].split(",")[0].split('\n')[
                                    0].lower().strip()
                        except Exception as e:
                            mylogger.error(f"error when split result: {result}")
                            mylogger.error(traceback.format_exc())
                        ...
                        if current_goal_finished not in ['yes', 'no']:
                            mylogger.error(
                                f"current_goal_finished should be yes or no, but got {current_goal_finished}")
                            current_goal_finished = 'no'

                        if waypoint_chosen not in BALL_COLORS[:len(new_ghost_nodes)] and waypoint_chosen not in [
                            'unknown',
                            'unneeded']:
                            mylogger.error(
                                f"waypoint_chosen should be in {BALL_COLORS[:len(new_ghost_nodes)]}, but got {waypoint_chosen}")
    ...
    def customize_rollout_top_down_map(self, mode, ml_weight=None, sample_ratio=None):
        if mode == 'train':
            feedback = 'sample'
        elif mode == 'eval' or mode == 'infer':
            feedback = 'argmax'
